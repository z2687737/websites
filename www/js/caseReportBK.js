...
/* pop-up help menu to user manual #Hlcampsites */

function showHelp(event, sectionId) {
  event.preventDefault(); // Prevent the default anchor behavior
  const helpContent = document.getElementById(sectionId).innerHTML;
...
// Call saveReportOffline() on form submit
...
/* Sync data to server - temporary disalbe else it keeps pop up */
async function XsyncDataToServer() {
...
async function fetchDataAndDisplay() {
    try {
      ...
      console.error('caseReport.js line 292 / 305 Error fetching data data.forEach is not a function:', error);
    }
  }
