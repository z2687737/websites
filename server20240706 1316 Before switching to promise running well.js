...
//before switching to promise running well
...
//VALIDATION RULES
// Check if fName, lname, and hPhone already exist in the database    
    const existingUser = await checkExistingUser(fName, lname, hPhone);
        if (existingUser) {
            return res.status(400).json({ error: 'User already registered with this name and phone number.' });
        }
    ...
    db.query(sql, registrationData, (error, results, fields) => {
      if (error) {
        console.error('Database query error:', error);
        res.status(500).json({ error: 'Registration failed - line 288' });
      } else {
        console.log('Registration successful - line 290');
        res.status(200).json({ message: 'Registration successful - line 291' });
      }
    });
