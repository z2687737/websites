...
//server.js Registration Route Registration Endpoint for registration table
...
app.post('/api/submitAttendance', (req, res) => {
  ...
  const volHours = (new Date(chkOutDTS) - new Date(chkInDTS)) / 3600000; // Convert milliseconds to hours
  const created_at = new Date();

  // Insert new attendance record
  const sqlInsertAttendance = 'INSERT INTO attendance (IDregistration, chkInDTS, chkOutDTS, volHours, created_at) VALUES (?, ?, ?, ?, ?)';
  db.query(sqlInsertAttendance, [IDregistration, chkInDTS, chkOutDTS, volHours, created_at], (err, result) => {
      if (err) {
          res.status(500).json({ error: err.message });
          return;
      }

      // Update accumulated volunteer hours in registration table
      const sqlUpdateRegistration = 'UPDATE registration SET aVolHours = aVolHours + ? WHERE IDregistration = ?';
      db.query(sqlUpdateRegistration, [volHours, IDregistration], (err, updateResult) => {
          if (err) {
              res.status(500).json({ error: err.message });
              return;
          }
          res.json({ message: 'Attendance recorded successfully', volHours });
      });
  });
});
